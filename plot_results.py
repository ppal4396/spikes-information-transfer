...
# count number of links and average TE after zero'ing non-sig TEs
n_links_mat = np.zeros((n_layers, n_layers))
avg_te_mat = np.zeros((n_layers, n_layers))
...
for filename in glob.glob(f'results/{mouse}/Layer*.csv'):
    ...
    for line in lines:
        ...
        if sig != '0.0':
            te = 0
        avg_te_mat[source_idx, dest_idx] += te

i = 0
while i < n_layers:
    j = 0
    while j < n_layers:
        avg_te_mat[i,j] = round(avg_te_mat[i,j] / n_links_mat[i,j], 2)
        j+=1
    i+=1
...
plot_matrix_colour_map(avg_te_mat, 'avg_te_rate')
